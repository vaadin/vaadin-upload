...
[propertyname]#accept#:: A filter for the types of files the user can upload. A filter is a comma-separated list of allowed [MIME-types](http://www.sitepoint.com/web-foundations/mime-types-complete-list/) or file-extensions.
Browsers use this information to filter out non-conforming files when prompting the user to select files.
Notice that although the ([HTML-4.01](https://www.w3.org/TR/html401/interact/forms.html#adef-accept)) MIME-types specification is widely supported,
only certain browsers accept the ([HTML5](https://www.w3.org/TR/html5/forms.html#file-upload-state-%28type=file%29)) file-extension syntax.

Nevertheless, this component will validate file types before sending them to the server.

[source,html]
----
<vaadin-upload max-files="3" accept="application/pdf,image/*" max-file-size="1000000"></vaadin-upload>
----
